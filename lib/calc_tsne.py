...
def PCA(X, INITIAL_DIMS):
    ...
    print >> sys.stderr, 'Performing PCA (%d -> %d)' % (X.shape[1], INITIAL_DIMS)
    U, s, v = linalg.svd(X.T)
    return X.dot(U[:,:INITIAL_DIMS])
...
def writeDat(dataMatrix, NO_DIMS, PERPLEX, landmarks):
    ...
    print >> sys.stderr, 'Dimension of projection : %i' % NO_DIMS
    print >> sys.stderr, 'Perplexity : %i' % PERPLEX
    print >> sys.stderr, 'Landmarks (ratio) : %f' % landmarks
...
def tSNE():
    ...
    """
    print >> sys.stderr,'Platform detected : %s' % sys.platform
    cmd = os.path.join(TSNE_DIRECTORY, 'tSNE_linux')
